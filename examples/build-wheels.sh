...
# each actor's own `wheels/` folder, so `docker build -t <tag> examples/<pair>/<actor>` (the
# same invocation `papeete-actor build` runs) stays a self-contained build with no
# `--build-context` flag and no change to `papeete-actor build`'s own fixed
# `docker build -t <tag> <folder>` (ADR-PA-0025).
#
# ACTORS ARE DISCOVERED, NOT LISTED — every folder under examples/<pair>/ holding a Dockerfile
# gets wheels, so a new example pair (another `examples/*/`) needs no edit here.
#
# Run once per change to any of the four packages, before building any actor's image.
set -euo pipefail
